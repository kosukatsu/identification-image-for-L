...
def main():
    ...
    layout = [
        ...
        [sg.Frame("正面から撮った写真ファイルと出力先を入力",[
            ...
            [sg.Text('入力画像'), sg.InputText(key="image_file",size=(52,5)), sg.FileBrowse("参照")],
        ...
        [sg.Frame("使用目的を選択するか画像サイズを直接入力",[
            ...
            [sg.Text("使用目的"), sg.Combo(purposes,default_value=purposes[2],key="selected_purpose",size=(46,5)), sg.Button("適用",key="bttn_apply_purpose")],
            [sg.Text("画像サイズ（縦x横[mm]）"),sg.InputText("40",size=(15,5),key="txt_size_y"),sg.Text("x"),sg.InputText("30",size=(15,5),key="txt_size_x")], 
        ])],
        [sg.Frame("オプション",[
            [sg.Text('モデル'), sg.Combo(methods,default_value=methods[0],key="model")],
            [sg.Text("解像度[dpi]"),sg.InputText("300",size=(47,5), key="dpi")],
            [sg.Text("拡大率(Dlibのみ)"),sg.Slider(range=(0.75,1.5),default_value=1,key="expansion",orientation="h",resolution=0.05)]
        ])],
    ...
    while True:
        ...
        if event == "detect_face":
            ...
            if values["model"]==methods[0]:
                detector = FaceDetectorDlib(expansion=float(values["expansion"])*1.5)
        ...
        if event=="output_image":
            ...
            output_image = spread_face(row_img, detector, id_size_x, id_size_y, dpi)
